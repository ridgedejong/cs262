...
router.get("/playergame/:key", readPlayerGame);
router.get("/playergame/gameid/:gameid", readGameID);
router.get("/playergame/playerid/:playerid", readPlayerID);
// End new
...
function readPlayerGame(req, res, next) {
    db.oneOrNone(`SELECT * FROM PlayerGame WHERE key=${req.params.key}`)
        .then(data => {
...
function readPlayerID(req, res, next) {
    db.many(`SELECT * FROM PlayerGame WHERE playerid=${req.params.playerid}`)
        .then(data => {
            returnDataOr404(res, data);
        })
        .catch(err => {
